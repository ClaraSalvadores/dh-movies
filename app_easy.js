...
let listadoDePeliculasArray =  movies.map((movie) => movie.title + "\n").sort();
let listadoDePeliculas = listadoDePeliculasArray.join("");


let piePagina = "Recordá que podés visitar las secciones:";
let piePaginaCartelera = "i. En Cartelera";
let piePaginaMásVotadas = "ii. Más Votadas";
let piePaginaSucursales = "iii. Sucursales";
let piePaginaContacto = "iv. Contacto";
let piePaginaFAQ ="v. Preguntas Frecuentes";

//En cartelera

let reseniasDePeliculasArray = movies.map (function(movie) {
	return movie.title +  "\n" + movie.overview + "\n" + "\n";
}).sort();
let reseniasDePeliculas = reseniasDePeliculasArray.join("");
...
//promedio 
let puntuacion = masVotadas.map(function(movie){
	return movie.vote_average;
});

let sumaPuntuacion = puntuacion.reduce(function(acu, cadaPuntuacion){
	return acu + cadaPuntuacion;
});
let promedioPuntuaciones= sumaPuntuacion/masVotadas.length;
let promedio = promedioPuntuaciones.toFixed(2);


let masVotadasDetalleArray = masVotadas.map (function(movie) {
	return "i.Titulo: "+ movie.title +  "\n" + "ii. Rating: "+ movie.vote_average + "\n" + "iii. Reseña: " + movie.overview + "\n" + "\n";
}).sort();

let masVotadasDetalle = masVotadasDetalleArray.join("");
...
let salasDetalleArray = theaters.map (function(sala) {
	return "i.Nombre "+ sala.name +  "\n" + "ii. Dirección: "+ sala.address + "\n" + "iii. Descripción: " + sala.description + "\n" + "\n";
});
let salasDetalle = salasDetalleArray.join("");

// Contacto
let contenidoContacto = "¿Tenés algo para contarnos? Nos encanta escuchar a nuestros clientes." + "\n" +  "Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales." +"\n" +  "Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible." + "\n" +  "Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.";

//PreguntasFrecuentes

let totalDePreguntas = faqs.length;

let preguntasDetalleArray = faqs.map (function(faq) {
	return "i.Pregunta: "+ faq.faq_title +  "\n" + "ii. Respuesta: "+ faq.faq_answer + "\n" + "\n";
});

let preguntasDetalle = preguntasDetalleArray.join("");
...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			res.end(tituloHome + "\n"+ "\n" + 
			"Total de Peliculas: " + totalDePeliculas+ "\n" +  "\n" +  
		...
		case '/mas-votadas':
			res.end("Más Votadas" + "\n"+ "\n" + 
			"Total de Peliculas: " + masVotadas.length +"\n" + "\n" + 
